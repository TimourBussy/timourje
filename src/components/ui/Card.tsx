      ...
      >
        ...
        >
          {isSmall && (
            <Heading
              level={5}
              className={`text-center md:text-left w-full ${!icon ? "ml-6" : ""}`}
            >
              {title}
            </Heading>
          )}
          {!isSmall && !(icon && iconDecoration) && (
            <Heading level={3} className="ml-6">
              {title}
            </Heading>
          )}
...
function ListItem({
  ...
  item: string | React.ReactNode | (string | React.ReactNode)[];
}) {
        ...
        <div className="relative">
          ...
          >
            ...
            <Heading
              level={!isSmall ? 3 : 5}
              className={`
								   ${isSmall ? "text-center md:text-left w-full flex-1" : ""}
								   ${!(icon && iconDecoration) ? "md:ml-4" : ""}
							   `}
            >
              {title}
            </Heading>
            ...
            >
              {listItems.map((item, index) => (
                <li
                  ...
                  className={`
										${!isSmall ? "p-3" : ""}
										text-center rounded-lg border cursor-default bg-linear-to-br transition-all hover:scale-105
      ...
      <Body>
        <div className="relative">
          <div className="flex items-center mb-2">
            ...
            <Heading
              level={3}
              className={`${!(icon && iconDecoration) ? "ml-6" : ""}`}
            >
              {title}
            </Heading>
          </div>
...
function Home({ icon, iconDecoration, title, description }: CardHomeProps) {
  return (
    ...
    >
      <div
        className="
					absolute top-0 right-0 w-32 h-32 rounded-bl-full bg-linear-to-br
      ...
      <Body>
        <div className="relative flex flex-col items-start w-auto">
          ...
          <Heading
            level={2}
            className={`text-3xl mt-4 mb-2 text-left ${!(icon && iconDecoration) ? "ml-6" : ""}`}
          >
            {title}
          </Heading>
